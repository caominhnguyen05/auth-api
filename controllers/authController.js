...
exports.verifyVerificationCode = async (req, res) => {
  ...
  try {
    const { error, value } = accceptCodeSchema.validate({
      email,
      providedCode,
    });
    if (error) {
    ...
    if (existingUser.verified) {
      return res
        .status(400)
        .json({ success: false, mesasge: "You are already verified!" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ success: false, mesasge: "Something is wrong with the code" });
    }
